    ...
    fn insert_rec(cur_node: &mut NodeType<T>, data: T) -> bool {
        ...
        if data < cur_node.as_ref().unwrap().data {
            match &cur_node.as_mut().unwrap().left {
                ...
                Some(present_node) => return Tree::<T>::insert_rec(&mut cur_node.as_mut().unwrap().left, data),
            }
        } else {
            match &cur_node.as_mut().unwrap().right { 
                ...
                Some(present_node) => return Tree::<T>::insert_rec(&mut cur_node.as_mut().unwrap().right, data),
            }
        }
    }
    ...
    fn contains_rec(cur_node: &NodeType<T>, data: T) -> bool {
        if data < cur_node.as_ref().unwrap().data {
            match &cur_node.as_ref().unwrap().left {
                None => return false,
                Some(present_node) => return Tree::<T>::contains_rec(&cur_node.as_ref().unwrap().left, data),
            }
        } else if data > cur_node.as_ref().unwrap().data {
            match &cur_node.as_ref().unwrap().right { 
                None => return false,
                Some(present_node) => return Tree::<T>::contains_rec(&cur_node.as_ref().unwrap().right, data),
            }
        }
        true
    }
